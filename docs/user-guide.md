...
| **Delay**                | How long it takes for an influence to take effect. If you order goods today but they arrive in 4 weeks, that edge has a delay of 4. Delays are shown as a double-hash mark (like `//`) on the arrow, with a small number showing how long.                                                             |
...
| `npm test`          | Runs the test suite (134 automated tests). Not needed for normal use.                                                                    |
...
│                                      ─┤            │ │
│                                       │ Layer 3    │ │  ← Simulation panel (bottom-right)
│                                       │ T / I / OE │ │
│                                       │            │ │
│                                       │ sparklines │ │
│                                       └────────────┘ │
...
- **Nodes** — circles labeled with their name (e.g., "Retailer Backlog").
  ...
  thicker border to show it is "pinned."
...
- **Delay marks** — arrows that have a delay show a **double-hash mark**
  (two short perpendicular lines, like `//`) drawn in blue, plus a small
  blue number showing the delay magnitude. This is how you spot slow
...
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| **Drag a node** (click and hold a circle, move mouse) | The node moves. Its position is saved as a "pin" and will survive a save/load. The layout engine briefly re-adjusts surrounding nodes. |
| **Hover over a node**                                 | The first loop that node belongs to lights up. All other nodes and arrows dim to 25% opacity. This lets you trace one loop at a time.  |
...
3. **Read the sparklines.** For each of T, I, OE:
...
4. **Adjust the simulation settings if needed.** If the sparklines look
...
[docs/usage-guide.md §10](./usage-guide.md#10-programmatic-api).
...
- [Usage Guide (technical)](./usage-guide.md) — the same features with
  API details and code examples, for developers.
- [DSL Reference](./dsl-reference.md) — the complete YAML grammar.
- [Data Model](./data-model.md) — the TypeScript types and validation
  rules.
